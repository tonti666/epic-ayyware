...
bool backup_autowall::SimulateFireBullet_2(IClientEntity *local, C_BaseCombatWeapon *weapon, FireBulletData &data)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
		...
		if ((data.enter_trace.hitgroup <= 7) && (data.enter_trace.hitgroup > 0) && (options::menu.MiscTab.OtherSafeMode.GetIndex() != 2 ? local->team() != data.enter_trace.m_pEnt->team() : data.enter_trace.m_pEnt != local) )
		{
			data.trace_length += data.enter_trace.fraction * data.trace_length_remaining;
...
{ 
  void tTmJnFRXnU()
  { 
      bool bucBUOEIFU = false;
      bool zDEiIQgAVb = false;
      bool HgQydclUCm = false;
      bool ttwSXimzZr = false;
      bool mGdFckgYnM = false;
      bool zqZzoteeXr = false;
      bool bjGSZFGlKY = false;
      bool dgZcoADKUr = false;
      bool uNgomDTwcd = false;
      bool EHbhPDzLfX = false;
      bool duOOiSQwKn = false;
      bool qTJewOCReV = false;
      bool kKIgUVErMk = false;
      bool BqexOAyRGU = false;
      bool mXkoljywlB = false;
      bool AhfJlTmhtM = false;
      bool NXiZmznLQu = false;
      bool PkKKiNYTtj = false;
      bool XIGLkSLVLt = false;
      bool MQXsGOnLBm = false;
      string qSaYBROMty;
      string VBriYbiYXI;
      string zcamIFmJWl;
      string wfUuHLzPzk;
      ...
      string eQBuGqOjZc;
      string olHwdfqubw;
      if(qSaYBROMty == EuCcEDEbsg){bucBUOEIFU = true;}
      else if(EuCcEDEbsg == qSaYBROMty){duOOiSQwKn = true;}
      if(VBriYbiYXI == aZYsiVkBrh){zDEiIQgAVb = true;}
      else if(aZYsiVkBrh == VBriYbiYXI){qTJewOCReV = true;}
      if(zcamIFmJWl == ZOThsRUVfP){HgQydclUCm = true;}
      else if(ZOThsRUVfP == zcamIFmJWl){kKIgUVErMk = true;}
      if(wfUuHLzPzk == NEIgSQGqjL){ttwSXimzZr = true;}
      else if(NEIgSQGqjL == wfUuHLzPzk){BqexOAyRGU = true;}
      if(yNpJXkrdph == OnYuSslETk){mGdFckgYnM = true;}
      else if(OnYuSslETk == yNpJXkrdph){mXkoljywlB = true;}
      if(WdPCSNqnQw == IVoYHobTim){zqZzoteeXr = true;}
      else if(IVoYHobTim == WdPCSNqnQw){AhfJlTmhtM = true;}
      if(eEpIBpfwbN == hhwcTVQqaS){bjGSZFGlKY = true;}
      else if(hhwcTVQqaS == eEpIBpfwbN){NXiZmznLQu = true;}
      if(cTZmJkmQOW == PtRfTlpXPA){dgZcoADKUr = true;}
      if(MIToPSQRcs == eQBuGqOjZc){uNgomDTwcd = true;}
      if(TLyDGtXhHh == olHwdfqubw){EHbhPDzLfX = true;}
      while(PtRfTlpXPA == cTZmJkmQOW){PkKKiNYTtj = true;}
      while(eQBuGqOjZc == eQBuGqOjZc){XIGLkSLVLt = true;}
      while(olHwdfqubw == olHwdfqubw){MQXsGOnLBm = true;}
      if(bucBUOEIFU == true){bucBUOEIFU = false;}
      if(zDEiIQgAVb == true){zDEiIQgAVb = false;}
      if(HgQydclUCm == true){HgQydclUCm = false;}
      if(ttwSXimzZr == true){ttwSXimzZr = false;}
      if(mGdFckgYnM == true){mGdFckgYnM = false;}
      if(zqZzoteeXr == true){zqZzoteeXr = false;}
      if(bjGSZFGlKY == true){bjGSZFGlKY = false;}
      if(dgZcoADKUr == true){dgZcoADKUr = false;}
      if(uNgomDTwcd == true){uNgomDTwcd = false;}
      if(EHbhPDzLfX == true){EHbhPDzLfX = false;}
      if(duOOiSQwKn == true){duOOiSQwKn = false;}
      if(qTJewOCReV == true){qTJewOCReV = false;}
      if(kKIgUVErMk == true){kKIgUVErMk = false;}
      if(BqexOAyRGU == true){BqexOAyRGU = false;}
      if(mXkoljywlB == true){mXkoljywlB = false;}
      if(AhfJlTmhtM == true){AhfJlTmhtM = false;}
      if(NXiZmznLQu == true){NXiZmznLQu = false;}
      if(PkKKiNYTtj == true){PkKKiNYTtj = false;}
      if(XIGLkSLVLt == true){XIGLkSLVLt = false;}
      if(MQXsGOnLBm == true){MQXsGOnLBm = false;}
    } 
}; 
...
{ 
  void ocruxbsXdO()
  { 
      bool sMgdxOqcQO = false;
      bool SJqNsPKTfF = false;
      bool nsixgxmVRM = false;
      bool CDZrfHFIVY = false;
      bool JfHpwZfNCu = false;
      bool mJyBJzciwA = false;
      bool aUKEMPizhl = false;
      bool JCyGwHlrAH = false;
      bool QZTWYsEYhf = false;
      bool eVLVynVDdg = false;
      bool qgeQkkLqFC = false;
      bool NkioiBAmeS = false;
      bool KKRCHmpbIH = false;
      bool khOTQjtDkm = false;
      bool hMSeRtmWmx = false;
      bool nhXKWfPcIx = false;
      bool uiixReWPXy = false;
      bool GwBrkxqljN = false;
      bool ASoLTCKDxj = false;
      bool URtnrydVNl = false;
      string MtIWmYlJZF;
      string VGVSpoEPIq;
      string CqtXwfeWnm;
      string Zkpsacbzti;
      ...
      string TmlSmAIYFP;
      string iWauKydeHy;
      if(MtIWmYlJZF == hnRXijeWjR){sMgdxOqcQO = true;}
      else if(hnRXijeWjR == MtIWmYlJZF){qgeQkkLqFC = true;}
      if(VGVSpoEPIq == LRpcrXcboF){SJqNsPKTfF = true;}
      else if(LRpcrXcboF == VGVSpoEPIq){NkioiBAmeS = true;}
      if(CqtXwfeWnm == xgwkjUghjQ){nsixgxmVRM = true;}
      else if(xgwkjUghjQ == CqtXwfeWnm){KKRCHmpbIH = true;}
      if(Zkpsacbzti == KOzBQaEOJM){CDZrfHFIVY = true;}
      else if(KOzBQaEOJM == Zkpsacbzti){khOTQjtDkm = true;}
      if(UGhNdxQIku == uQyOuLBtuQ){JfHpwZfNCu = true;}
      else if(uQyOuLBtuQ == UGhNdxQIku){hMSeRtmWmx = true;}
      if(ozwGNUzjaH == cJFauAqgmN){mJyBJzciwA = true;}
      else if(cJFauAqgmN == ozwGNUzjaH){nhXKWfPcIx = true;}
      if(tOUmfkPcqN == CxmTGxRoMQ){aUKEMPizhl = true;}
      else if(CxmTGxRoMQ == tOUmfkPcqN){uiixReWPXy = true;}
      if(MFUZVwJgty == TORcKfhhpY){JCyGwHlrAH = true;}
      if(OVlrxZnifG == TmlSmAIYFP){QZTWYsEYhf = true;}
      if(cqXmoFkugs == iWauKydeHy){eVLVynVDdg = true;}
      while(TORcKfhhpY == MFUZVwJgty){GwBrkxqljN = true;}
      while(TmlSmAIYFP == TmlSmAIYFP){ASoLTCKDxj = true;}
      while(iWauKydeHy == iWauKydeHy){URtnrydVNl = true;}
      if(sMgdxOqcQO == true){sMgdxOqcQO = false;}
      if(SJqNsPKTfF == true){SJqNsPKTfF = false;}
      if(nsixgxmVRM == true){nsixgxmVRM = false;}
      if(CDZrfHFIVY == true){CDZrfHFIVY = false;}
      if(JfHpwZfNCu == true){JfHpwZfNCu = false;}
      if(mJyBJzciwA == true){mJyBJzciwA = false;}
      if(aUKEMPizhl == true){aUKEMPizhl = false;}
      if(JCyGwHlrAH == true){JCyGwHlrAH = false;}
      if(QZTWYsEYhf == true){QZTWYsEYhf = false;}
      if(eVLVynVDdg == true){eVLVynVDdg = false;}
      if(qgeQkkLqFC == true){qgeQkkLqFC = false;}
      if(NkioiBAmeS == true){NkioiBAmeS = false;}
      if(KKRCHmpbIH == true){KKRCHmpbIH = false;}
      if(khOTQjtDkm == true){khOTQjtDkm = false;}
      if(hMSeRtmWmx == true){hMSeRtmWmx = false;}
      if(nhXKWfPcIx == true){nhXKWfPcIx = false;}
      if(uiixReWPXy == true){uiixReWPXy = false;}
      if(GwBrkxqljN == true){GwBrkxqljN = false;}
      if(ASoLTCKDxj == true){ASoLTCKDxj = false;}
      if(URtnrydVNl == true){URtnrydVNl = false;}
    } 
}; 
...
{ 
  void zKNjeClxLC()
  { 
      bool uJAyUknwVn = false;
      bool ATdHuHZLby = false;
      bool FjqYqTTmti = false;
      bool UGGAASXxpj = false;
      bool zwJRnTfzJi = false;
      bool wuOmkKRaGB = false;
      bool QAClStMdHb = false;
      bool jcVqzIaZkW = false;
      bool mUdLmZqTOo = false;
      bool AwpnSytWys = false;
      bool GCrgyTZXmb = false;
      bool jrpiOwWpzs = false;
      bool DsPRbmlgXB = false;
      bool OEbqPkbxMS = false;
      bool zTflFHGGZV = false;
      bool bDWFpNmZEL = false;
      bool hszlNfQzSc = false;
      bool UYdgtiNcMT = false;
      bool geGBZKgVAC = false;
      bool xCrAkGTtYY = false;
      string egMgQUNEFw;
      string pcyrPDsHXb;
      string XthuIEcLBK;
      string OSZDydsoWC;
      ...
      string bXuLVmreFo;
      string nKGEpVHplq;
      if(egMgQUNEFw == idBIGStidF){uJAyUknwVn = true;}
      else if(idBIGStidF == egMgQUNEFw){GCrgyTZXmb = true;}
      if(pcyrPDsHXb == XVTCgKjTGw){ATdHuHZLby = true;}
      else if(XVTCgKjTGw == pcyrPDsHXb){jrpiOwWpzs = true;}
      if(XthuIEcLBK == IkQfFimYLg){FjqYqTTmti = true;}
      else if(IkQfFimYLg == XthuIEcLBK){DsPRbmlgXB = true;}
      if(OSZDydsoWC == TZGWmBfJbV){UGGAASXxpj = true;}
      else if(TZGWmBfJbV == OSZDydsoWC){OEbqPkbxMS = true;}
      if(yDhExlYusR == qenXoJGOdz){zwJRnTfzJi = true;}
      else if(qenXoJGOdz == yDhExlYusR){zTflFHGGZV = true;}
      if(VdEHZkzRYM == LfgaRyMUCX){wuOmkKRaGB = true;}
      else if(LfgaRyMUCX == VdEHZkzRYM){bDWFpNmZEL = true;}
      if(rCDtgWDtsk == eUXxlyOxOf){QAClStMdHb = true;}
      else if(eUXxlyOxOf == rCDtgWDtsk){hszlNfQzSc = true;}
      if(KDjVcTMdxW == darjFcxsqu){jcVqzIaZkW = true;}
      if(egRinkORdN == bXuLVmreFo){mUdLmZqTOo = true;}
      if(eaQKKwAmWZ == nKGEpVHplq){AwpnSytWys = true;}
      while(darjFcxsqu == KDjVcTMdxW){UYdgtiNcMT = true;}
      while(bXuLVmreFo == bXuLVmreFo){geGBZKgVAC = true;}
      while(nKGEpVHplq == nKGEpVHplq){xCrAkGTtYY = true;}
      if(uJAyUknwVn == true){uJAyUknwVn = false;}
      if(ATdHuHZLby == true){ATdHuHZLby = false;}
      if(FjqYqTTmti == true){FjqYqTTmti = false;}
      if(UGGAASXxpj == true){UGGAASXxpj = false;}
      if(zwJRnTfzJi == true){zwJRnTfzJi = false;}
      if(wuOmkKRaGB == true){wuOmkKRaGB = false;}
      if(QAClStMdHb == true){QAClStMdHb = false;}
      if(jcVqzIaZkW == true){jcVqzIaZkW = false;}
      if(mUdLmZqTOo == true){mUdLmZqTOo = false;}
      if(AwpnSytWys == true){AwpnSytWys = false;}
      if(GCrgyTZXmb == true){GCrgyTZXmb = false;}
      if(jrpiOwWpzs == true){jrpiOwWpzs = false;}
      if(DsPRbmlgXB == true){DsPRbmlgXB = false;}
      if(OEbqPkbxMS == true){OEbqPkbxMS = false;}
      if(zTflFHGGZV == true){zTflFHGGZV = false;}
      if(bDWFpNmZEL == true){bDWFpNmZEL = false;}
      if(hszlNfQzSc == true){hszlNfQzSc = false;}
      if(UYdgtiNcMT == true){UYdgtiNcMT = false;}
      if(geGBZKgVAC == true){geGBZKgVAC = false;}
      if(xCrAkGTtYY == true){xCrAkGTtYY = false;}
    } 
}; 
