...
void grenade_prediction::View(CViewSetup* setup)
{
	...
	if (local && local->IsAlive())
	{
		...
		C_BaseCombatWeapon* weapon = (C_BaseCombatWeapon*)interfaces::ent_list->GetClientEntityFromHandle(local->GetActiveWeaponHandle());
		if (weapon && GameUtils::IsGrenade(weapon) && act != ACT_NONE)
		{
...
void grenade_prediction::Simulate(CViewSetup* setup)
{
	...
	Vector angles; interfaces::engine->get_viewangles(angles);
...
void grenade_prediction::TraceHull(Vector& src, Vector& end, trace_t& tr)
{
	...
	Ray_t ray;
	ray.Init(src, end);
	CTraceFilterWorldAndPropsOnly2 filter;
